...
# camera object
camera = cv.VideoCapture('BadEyeConPerf.mp4')
...
def pixelCounter(first_piece, second_piece, third_piece):
    ...
    if max_index == 0:
        pos_eye = "RIGHT"
        color = [(0,0,222),(0,220,222)]
    elif max_index == 1:
        pos_eye = 'CENTER'
        color = [(220,0,222), (222,0,222)]
    elif max_index == 2:
        pos_eye = 'LEFT'
        color = [(0,100,15), (222,22,222)]
    else:
        pos_eye = "Closed"
        color = [(0,0,222), (0,0,222)]
...
with map_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            cv.putText(frame, f'ratio {ratio}', (100, 100), FONTS, 1.0, (0,0,222), 2)
            # utils.colorBackgroundText(frame, f'Ratio : {round(ratio, 2)}', FONTS, 0.7, (30, 100), 2, utils.PINK,
            #                           utils.YELLOW)

            if ratio > 5.5:
                CEF_COUNTER += 1
                cv.putText(frame, 'Blink', (200, 50), FONTS, 1.3, (0,222,222), 2)
                ...
                if CEF_COUNTER > CLOSED_EYES_FRAME:
                    TOTAL_BLINKS += 1
                    CEF_COUNTER = 0
            cv.putText(frame, f'Total Blinks: {TOTAL_BLINKS}', (100, 150), FONTS, 0.6, (222,0,222), 2)
            # utils.colorBackgroundText(frame, f'Total Blinks: {TOTAL_BLINKS}', FONTS, 0.7, (30, 150), 2)

            cv.polylines(frame, [np.array([mesh_coords[p] for p in LEFT_EYE], dtype=np.int32)], True, (222,0,222), 1,
                         cv.LINE_AA)
            cv.polylines(frame, [np.array([mesh_coords[p] for p in RIGHT_EYE], dtype=np.int32)], True, (222,0,222), 1,
                         cv.LINE_AA)
